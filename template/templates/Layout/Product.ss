<section id="product" class="home clearfix pt-md-4 pb-md-6">
   <div class="container">
		<div class="row">
			...
			<div class="offset-md-1 col-md-7 col-sm-6">
				<!--<nav id="projectsNav">
				  <ul class="pager">
					...
					<div class="row no-gutters">
						<div class="col-md-12">
							<% if $Preise && $ShowPricingTable %>
								...
								<% else %>
								<% if $Price >0 %><h3>$formattedNumber($Price) &euro; / kg *</h3><% end_if %>
					...
					<% if $Preise %>
						<div class="col-md-6 pr-md-2">
							...
							<div class="selectric-wrapper">
								...
								<% loop $Preise %>
									<option value="$ID" data-price="$Price" data-vac="$Vacuum" <% if $Top.loadSelectedParameters.Variant01==$ID %> selected="selected"<% else_if $IsFirst %> selected="selected"<% end_if %>><% if $ShowContent %>$Content<% else %>$DisplayAmount<% end_if %></option>
								<% end_loop %>
								</select>
							</div>
...
function refreshSelectedProduct(){
		if(jQuery('#variant01 option[value=' + jQuery('#variant01').val() + ']').attr('data-vac')=="1"){
			jQuery('#vac_container').css('display','block');
		}else{
			jQuery('#vac_container').css('display','none');
		}
