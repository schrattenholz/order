					...
					<% if $Preise %>
						<div class="col-md-6 pr-md-2">
							...
							<div class="selectric-wrapper">
								...
								<% loop $Preise %>
									<option value="$ID" data-price="$Price" data-vac="$Vacuum" <% if $Top.loadSelectedParameters.Variant01==$ID %> selected="selected"<% else_if $First %> selected="selected"<% end_if %>><% if $ShowContent %>$Content<% else %>$DisplayAmount<% end_if %></option>
								<% end_loop %>
								</select>
							</div>
